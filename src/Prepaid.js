...
export class Prepaid extends Component {
	constructor(props) {
		super(props);
	...
	componentDidMount() {
		const url = 'http://localhost:8010/home/prepaid';    // url to get prepaid details from backend
		let params = queryString.parse(this.props.location.search);
		
		let data = {
			"user": params.id    // assign user Id
		}
		let fetchData = {
			method: 'POST',
		...
		fetch(url, fetchData)
			.then((resp) => resp.json())     // get response from back end
			.then((resp) => {
				this.setState({       //assign the data obtained to local variables
	...
	render() {
		...
		return (
			...
			<div>
				<div style={container}>
					<div style={H1}>PREPAID</div>
					<div style={{ paddingTop: 15 }} />
					<div style={row}>
						<div style={H2}>Balance</div>
						<div style={row}>
							<div>{this.state.Balance} INR </div>
						</div>
					</div>
					<div style={{ paddingTop: 15 }} />
